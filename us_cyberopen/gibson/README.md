...
No remote connection, Manual Submission Only
...
<h2>Overview</h2>
This challenge was a hard pwn from US Cyber Open. The exploit was actually not that complicated but the fact that it used the s390 architecture made it challenging. The lack of decent documentation and tooling support made this challenge unfamiliar and getting the local environment to work was a pain. 
...
- Run `docker run --rm --privileged multiarch/qemu-user-static --reset -p yes` as specified in the tips.md
...
The challenge had a nice setup with a competitor port that stimulated the challenge and a debug port that had gdb enabled
    ...
    - 9999: Fake Infrastructure 
    - 8888: Debug port, gdb-server on 1234 


This gdb was for the s390x architecture so be sure to install gdb-multi-arch. GEF had a lot of issues so I ended up disabling most of its features.

<h2>s390x</h2>
The s390x is a 64 bit big endian architecture developed by IBM. Most of the instructions can be found here http://www.tachyonsoft.com/inst390m.htm. Here's a list of the registers that look very similar to Arm registers
...
Googling Ghidra S390X resulted in this Chinese ctf blog post https://gange666.github.io/2019/09/09/Bytectf_2019_s390_Writeup. Except the line with ghidra was not a setup guide, rather it was a note that none of the disassembling tools worked normally. Major disappointment but that means we only have objdump. Thankfully the program seems pretty simple

Running `s390x-linux-gnu-objdump -d bin/mainframe` can give us the full disassemly

Most of these instructions looked foreign but with the instructions lookup sheet and running in gdb we can get a general overview of the program execution.
- Main first calls setvbuf to fix buffering issues (Common CTF thing, not important) 
- Prints out banner 
- Reads in an input 
- Transforms out input 
- Printf's the transformed input
- Sleep + Exit
Using this we can map most of the instructions in main to fit this execution flow
```
...
In this loop, all of these instructions seem like loads except one, xilf with XOR. It is xoring whatever is in r1 with 82 and then moving that into the buffer. We can confirm this by XORing our input with 82 and checking to see if we get the same input.
```
 10008e2:       e3 10 b4 a0 00 04       lg      %r1,1184(%r11)
 10008e8:       43 11 b0 a0             ic      %r1,160(%r1,%r11)
 10008ec:       c0 17 00 00 00 52       xilf    %r1,82
...
<h3>Attempt 1</h3>
If this was x86 the exploit path would be relatively simple, use printf to leak values and then overwrite GOT. The main difference is that I couldn't find the magically one_gadget. This means that I'll have to resort to replacing something in the GOT with system. Printf is a great candidate as our input is directly passed so if we input /bin/sh we should get a shell.
The first issue is that after printf will exit so we need a way to loop the program. Luckily with our buffer overflow, we can overwrite the return address with main so it will loop forever.

Using GDB we can find the offset by spamming characters and checking which ones end up as the instructions pointer. After connecting to port 8888 it will hang until you attach to the gdb server on port 1234.

After inputting a cyclic pattern gdb crashes and we can find the offset as well as what register control we have.
```
...
With this offset, we can create a payload that will leak values and then loop the program
```python 
bof_offset = 1144
payload = b''
...
After the first time, this payload is sent the return pointer will remain overwritten so we can get unlimited attempts at printf. 
My first exploit attempt took advantage of a partial GOT overwrite as printf and system were very close in this libc. 
...
Doing a 32 bit overwrite was not possible as printing that many characters took a couple of minutes and there was a 1-minute timeout on the remote instance
However, we can do a 16 bit overwrite to just change the last 2 bits of the GOT entry for printf. This was a very bad idea as most of the time alsr base caused the difference to leak into the next byte. We would need to do some brute forcing to get a good libc address. 
```
...
- Use printf padding to print bytes equal to libc.symbols.system & 0xffff
- Use "%hn" to write to 16 bits of printf got address
- Send /bin/sh xored
...
I had no idea what this does but gdb showed that it was just moving values from r15(the stack)+1280 into registers from r11-r15. 
```
gef>  x/4i $pc
=> 0x100093a <main+266>:        lmg     %r11,%r15,1280(%r11)
   0x1000940 <main+272>:        br      %r14
...
Hence if we can control the stack we can use this as a "ret" like in ROP. Now if we call system we only need to control arg1 which is r2. After the overflow, we have control of r11-15 so if we can get a move instruction to move something from those registers to r2 we can call the system.
Using objdump I dumped the entire libc and then grepped through it looking for instructions that moved r11 into r2. The best scenario is that after this it immediately calls our "ret".

```
grep -r l*l.*r2.*r11 libc.disasm -A 2 |  grep  "br..r14" -B 2
```

To my surprise, this actually existed in the binary and at this point, we were done.
...
- r15 NEEDS to be a valid address that can be used as the stack. Using the BSS didn't work for me (it did for some others) so I ended up leaking libc's environ to find the stack. From there I calculated the offset to the current stack pointer, I always used this pointer as it worked nicely. 
...
Lessons Learned:
- Never assume addresses, if you have a leak always validate them
- When learning a new architecture try to map instructions to ones that you are already familiar with 
- GDB's your best friend for learning new architectures 
